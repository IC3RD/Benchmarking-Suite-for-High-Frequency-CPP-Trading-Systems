...
TEST_F(OrderDataTests, getOrderType) {
  ASSERT_EQ(getOrderData().getOrderType(), OrderTypes::ASK);
}

TEST_F(OrderDataTests, getExchange) {
  ASSERT_EQ(getOrderData().getExchange(), Exchange::BINANCE);
}

TEST_F(OrderDataTests, getOrderPrice) {
  ASSERT_EQ(getOrderData().getOrderPrice(), 100);
}

TEST_F(OrderDataTests, getOrderVolume) {
  ASSERT_EQ(getOrderData().getOrderVolume(), 1);
}