...
struct ListenerTests : public ::testing::Test {
    // test the implementation of the json parsing and populating the orderbook correctly
    
  std::shared_ptr<OrderBook> orderBook;
  std::shared_ptr<FileReaderListener> listener;

    FileReaderListener& getListener() {
        return *listener;
    }
  ...
  virtual void SetUp() override {
      ...
      listener = std::make_shared<FileReaderListener>(*orderBook, Exchange::DUMMY, "");   
  }
...
TEST_F(ListenerTests, jsonAddOneAsk) { 
    nlohmann::json json = nlohmann::json::object({ { "ask", { {"price", 2}, {"volume", 1} } } });
    getListener().passJSON(json);
...
TEST_F(ListenerTests, jsonAddOneBid) { 
    nlohmann::json json = nlohmann::json::object({ { "bid", { {"price", 2}, {"volume", 1} } } });
    getListener().passJSON(json);
...
TEST_F(ListenerTests, jsonAddOneBidAndOneAsk) { 
    nlohmann::json json = nlohmann::json::object({ { "ask", { {"price", 2}, {"volume", 1} } }, { "bid", { {"price", 2}, {"volume", 1} } } });
    getListener().passJSON(json);
    ASSERT_TRUE(getOrderBook().hasBids());
    ASSERT_TRUE(getOrderBook().hasAsks());
}