...
void Band::insertNewData(long price, std::shared_ptr <OrderBook> book) {
    /*if (price > book->getLowestAsk()->getOrderPrice() + stdDeviation || price < book->getHighestBid()->getOrderPrice() - stdDeviation) {
        return;
        //this is just a measure to ensure data that is far from our bands is not affected
    }
     */
