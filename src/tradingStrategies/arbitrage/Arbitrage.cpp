...
void Arbitrage::runStrategy() {
    for (int i = 0; i < exchanges.size() - 1; ++i) {
        for (int j = i + 1; j < exchanges.size(); ++j) {
            process(exchangeOrderBooks.find(exchanges.at(i))->second, exchangeOrderBooks.find(exchanges.at(j))->second);
        }
    }
}

void Arbitrage::process(std::shared_ptr <OrderBook> exchange1Book, std::shared_ptr <OrderBook> exchange2Book) {
    int fee1 = 0.01;
    int fee2 = 0.01;
    ...
    if (exchange1SellPrice > exchange2BuyPrice) {
        if (exchange1SellPrice * volume - exchange1SellPrice * volume * fee1 -
            ...
            if (exchange2Book->getAskAt(0)->getOrderVolume() < volume) {
                if (exchange1SellPrice > exchange2Book->getAskAt(1)->getOrderPrice()) {
                    ...
                    // Since these are limit orders we will fill at the highest ask and then continue
                    // buying at the lower price since there is still an arbitrage opportunity
                }
                ...
                sell(exchange1Book->getHighestBid());
                buy(exchange2Book->getLowestAsk());  // these should be market orders to ensure fill
            }
        }
        ...
        if (exchange2SellPrice * volume - exchange2SellPrice * volume * fee2 -
            ...
            if (exchange1Book->getAskAt(0)->getOrderVolume() < volume) {
                ...
                sell(exchange2Book->getHighestBid());
                buy(exchange1Book->getLowestAsk());  // these should be market orders to ensure fill
            }
        }
    }
};
