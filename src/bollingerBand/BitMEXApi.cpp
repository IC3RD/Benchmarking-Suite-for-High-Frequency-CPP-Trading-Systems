...
string BitMEXApi::generatePostFields(Order order) {
    ...
    output += "symbol=" + currency + "&side=" + (order.isBuyOrder() ? "Buy" : "Sell") + "&orderQty=" +
            to_string(quantity) + "&price=" + to_string(order.getPrice()) + "&ordType=Limit";

    return output;
}

void BitMEXApi::placeOrder(Order order) {
    //curl -X POST --header 'Content-Type: application/x-www-form-urlencoded' --header 'Accept: application/json' --header 'X-Requested-With: XMLHttpRequest' -d 'symbol=XBTUSD&side=Buy&orderQty=1&price=55&ordType=Limit' 'https://www.bitmex.com/api/v1/order'
