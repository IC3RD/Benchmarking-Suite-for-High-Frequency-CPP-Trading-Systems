...
int main() {
  ...
  // Setup a callback to be fired (in a background thread, watch out for race
  // conditions !) when a message or an event (open, close, error) is received
  webSocket.setOnMessageCallback([&](const ix::WebSocketMessagePtr& msg) {
    if (msg->type == ix::WebSocketMessageType::Message) {
      ...
      if (j.contains("data")) {
        if (!data) {
          data = new MarketData("Bitcoin", j.at("data")[0].at("askPrice"),
                                j.at("data")[0].at("bidPrice"),
                                j.at("data")[0].at("volume"));
          data->newMarketData();
        } else {
