...
curl_fail_init::curl_fail_init(error_mode mode) { test_failure_mode = mode; }

curl_fail_init::~curl_fail_init() { test_failure_mode = success; }
...
// This function signature is required to mock curl, so disable a warning
// generated from it.
...
#pragma clang diagnostic pop
  ...
  switch (option) {
    case CURLOPT_HEADERFUNCTION:
      // Set client::write_header as the function to be called as soon as mock
      // curl has received header data.
      if (h->test_failure_mode ==
          curl_impl::error_mode::header_function_error) {
        va_end(vl);
    ...
    case CURLOPT_HEADERDATA:
      // Pointer to the context to write the header part of the received data
      // to.
    ...
    case CURLOPT_WRITEFUNCTION:
      // Set client::write_body as the function to be called as soon as mock
      // curl has received data.
      if (h->test_failure_mode ==
          curl_impl::error_mode::write_body_function_error) {
        va_end(vl);
        return CURLE_COULDNT_CONNECT;
      }
    ...
    case CURLOPT_WRITEDATA:
      ...
      if (h->test_failure_mode ==
          curl_impl::error_mode::write_body_context_error) {
        va_end(vl);
        return CURLE_COULDNT_CONNECT;
      }
    ...
    case CURLOPT_READFUNCTION:
      // Set client::read_body as the function to be called for mock curl to
      // read the request body.
      if (h->test_failure_mode ==
          curl_impl::error_mode::read_body_function_error) {
        va_end(vl);
        return CURLE_COULDNT_CONNECT;
      }
    ...
    case CURLOPT_READDATA:
      // Pointer to the context to read the request body from by the
      // READFUNCTION callback.
      if (h->test_failure_mode ==
          curl_impl::error_mode::read_body_context_error) {
        va_end(vl);
        return CURLE_COULDNT_CONNECT;
      }
    ...
    case CURLOPT_HTTPHEADER:
      // Set the mock curl list of custom headers to that which was passed in
      // the request.
      ...
      break;
    case CURLOPT_COOKIE:
      // Set the mock curl Cookie header to that which was passed in the
      // request.
      ...
      break;
    case CURLOPT_CAINFO:
      // Set the mock curl Certificate Authority path to that which was passed
      // in the request.
    ...
    case CURLOPT_SSLCERT:
      // Set the mock curl SSL client cert name to that which was passed in the
      // request.
    ...
    case CURLOPT_PROXY:
      ...
      break;
    case CURLOPT_SSLKEY:
      // Set the mock curl private keyfile name to that which was passed in the
      // request.
    ...
    case CURLOPT_CONNECTTIMEOUT_MS:
      if (h->test_failure_mode ==
          curl_impl::error_mode::connect_timeout_error) {
        va_end(vl);
        return CURLE_COULDNT_CONNECT;
      }
    ...
    case CURLOPT_TIMEOUT_MS:
      if (h->test_failure_mode ==
          curl_impl::error_mode::request_timeout_error) {
        va_end(vl);
        return CURLE_COULDNT_CONNECT;
      }
      break;
...
CURLcode curl_easy_perform(CURL *easy_handle) {
  ...
  static const array<string, 3> VALID_URLS{{"http://valid.com/",
                                            "https://download.com",
                                            "https://remove_temp_file.com"}};

  /*
   * If we pass 'valid.com' in the test, return HTTP status 200. Otherwise,
   * return status 404.
   */
  if (h->write_header) {
    bool is_valid_url = find(VALID_URLS.begin(), VALID_URLS.end(),
                             h->request_url) != VALID_URLS.end();
    if (is_valid_url) {
      ...
      h->write_header(&header_content[0], 1, header_content.size(),
                      h->header_context);
  ...
  /*
   * For file download. It is OK if exception is thrown if write_body is not
   * set, that means something went wrong in our code's setup so we want our
   * test to fail.
   */
  if (h->request_url == "https://download.com" ||
      h->request_url == "https://download_trigger_404.com") {
    string download_msg = (h->request_url == "https://download.com")
                              ? "successfully downloaded file"
                              : "Not found";
    h->write_body(const_cast<char *>(download_msg.c_str()), 1,
                  reinterpret_cast<size_t>(download_msg.size()),
                  h->body_context);
