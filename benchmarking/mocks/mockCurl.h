...
struct curl_impl {
  enum struct http_method { get, put, post };
  ...
  void*
      header_context;  // Where to write the header part of the received data to
  ...
  std::string request_url, cookie, cacert, client_cert, client_key, client_crl,
      proxy;
  long protocols;
  ...
  curl_slist*
      header;  // List of custom request headers to be passed to the server

  std::string read_buffer;  // Buffer to test reading the request body
  std::string resp_body;  // Response body which should be written to a context
                          // using the write_body function callback
...
enum error_mode { success, easy_init_error, global_init_error };
